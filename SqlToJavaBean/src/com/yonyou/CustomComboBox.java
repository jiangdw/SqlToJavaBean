...
public class CustomComboBox<E> extends JComboBox<E> {
		...
		comboBox.addItemListener(new ItemListener() {
			public void itemStateChanged(ItemEvent e) {
				if (ItemEvent.SELECTED == e.getStateChange()) {
					System.out.println("选择的�?�：" + comboBox.getSelectedItem());
				}
			}
		});
	...
	/**
	 * 如果想将ComboBox的Background和Foreground与弹出的Popup的Background和Foreground保持�?致则可以调用此方�?
	 */
	...
	/**
	 * 如果想将ComboBox的Background和Foreground和Border与弹出的Popup的Background和Foreground和Border保持�?致则可以调用此方�?
	 */
	...
	public static class CustomComboBoxUI extends MetalComboBoxUI {
		...
		public void paintCurrentValue(Graphics g, Rectangle bounds, boolean hasFocus) {
			Component c = comboBox.getRenderer().getListCellRendererComponent(listBox, comboBox.getSelectedItem(), -1, hasFocus && !isPopupVisible(comboBox),
					false);
			c.setBackground(comboBox.getBackground());// 清除渲染器原来设置的背景�?,将渲染器的背景设置成ComboBox的背景色
			c.setForeground(comboBox.getForeground());// 清除渲染器原来设置的前景�?,将渲染器的前景设置成ComboBox的前景色
